    ...
    try:
        for table_bucket_page in table_buckets_paginator.paginate():
            for table_bucket in table_bucket_page["tableBuckets"]:
                ...
                # Iterate all tables
                tables_paginator = s3_client.get_paginator("list_tables")
                for tables_page in tables_paginator.paginate(tableBucketARN=table_bucket["arn"]):
                    for table in tables_page["tables"]:

                        # Fetch table policy
                        try:
                            get_table_policy_response = s3_client.get_table_policy(
