...
def analyze(account_id, region, boto_session, boto_config, policy_analysis_function):
    ...
    list_access_points_for_directory_buckets_paginator = s3_client.get_paginator(
        "list_access_points_for_directory_buckets"
    )

    # Iterate all directory bucket access points
    for access_points_page in list_access_points_for_directory_buckets_paginator.paginate(AccountId=account_id):
        for access_point in access_points_page["AccessPointList"]:

            # Fetch the directory bucket access point policy
            try:
                ...
                # This directory bucket access point does not have a policy configured
                continue
